...
export const usePayments = (
  ...
  const {
    data: Intent,
    isPending: creatingIntent,
    error: intentError,
  } = useQuery({
    queryKey: ["payment-intent"],
    queryFn: async () => {
      const result = await onGetStripeClientSecret()
  ...
  const { mutateAsync: createGroup, isPending } = useMutation({
    mutationFn: async (data: z.infer<typeof CreateGroupSchema>) => {
      if (!stripe || !elements || !Intent || !Intent.secret) {
        toast("Error", {
      ...
      try {
        ...
        if (paymentIntent?.status === "succeeded") {
          ...
          if (created && created.status === 200) {
            ...
            throw new Error(created?.message || "Failed to create group")
          }
        } else {
