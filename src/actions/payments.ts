...
export const onGetStripeClientSecret = async () => {
  try {
    ...
    if (paymentIntent && paymentIntent.client_secret) {
      ...
      return { status: 200, secret: paymentIntent.client_secret }
    } else {
      console.error("Payment intent created but client secret is missing")
...
// Debugging function to check Stripe configuration
export const debugStripeConfig = async () => {
  try {
    const balance = await stripe.balance.retrieve()
    console.log("Stripe connection successful. Balance:", balance)
    return { status: 200, message: "Stripe configuration is valid" }
  } catch (error) {
    console.error("Stripe configuration error:", error)
    return { status: 500, message: "Stripe configuration is invalid" }
  }
}
